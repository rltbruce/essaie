...
class Demande_debut_travaux_moe extends REST_Controller {
    ...
    public function index_get() 
    {
       ...
       if ($menu=="getdemandedisponibleBycontrat")
        ...
        {
            $tmp = $this->Demande_debut_travaux_moeManager->finddemandevalideBycontrat($id_contrat_bureau_etude);
            if ($tmp) 
            {
                foreach ($tmp as $key => $value) 
                {
        ...
        {
            $tmp = $this->Demande_debut_travaux_moeManager->finddemandeemidpfiByIdcontrat($id_contrat_bureau_etude);
            if ($tmp) 
            {
                foreach ($tmp as $key => $value) 
                {
                   ...
                   $data[$key]['id'] = $value->id;
                    ...
                    $data[$key]['date'] = $value->date;
                     $data[$key]['validation'] = $value->validation;
                    $data[$key]['contrat_bureau_etude'] = $contrat_bureau_etude;
                    //$data[$key]['ato'] = $id_contrat_bureau_etude;
                }
            } 
                else
                    $data = array();
        ...
        {
            $tmp = $this->Demande_debut_travaux_moeManager->findcreerByIdcontrat($id_contrat_bureau_etude);
            if ($tmp) 
            {
                foreach ($tmp as $key => $value) 
                {
       ...
       /* if ($menu=="getdemandedisponibleBycontrat")
        ...
        {
            $tmp = $this->Demande_debut_travaux_moeManager->findAllInvalideBycisco($id_cisco);
            if ($tmp) 
            {
                foreach ($tmp as $key => $value) 
                {
        ...
        {
            $tmp = $this->Demande_debut_travaux_moeManager->findAllBycontrat($id_contrat_bureau_etude);
            if ($tmp) 
            {
                foreach ($tmp as $key => $value) 
                {
        ...
        {
            $tmp = $this->Demande_debut_travaux_moeManager->findAllValide();
            if ($tmp) 
            {
                foreach ($tmp as $key => $value) 
                {
        ...
        {
            $tmp = $this->Demande_debut_travaux_moeManager->findAllValidedpfi();
            if ($tmp) 
            {
                foreach ($tmp as $key => $value) 
                {
        ...
        {
            $tmp = $this->Demande_debut_travaux_moeManager->findAllValidebcaf();
            if ($tmp) 
            {
                foreach ($tmp as $key => $value) 
                {
        ...
        {
            $tmp = $this->Demande_debut_travaux_moeManager->findAllInvalide();
            if ($tmp) 
            {
                foreach ($tmp as $key => $value) 
                {
        ...
        {
            $tmp = $this->Demande_debut_travaux_moeManager->findAllInvalideBycontrat($id_contrat_bureau_etude);
            if ($tmp) 
            {
                foreach ($tmp as $key => $value) 
                {
        ...
        {
            $data = array();
            $demande_debut_travaux_moe = $this->Demande_debut_travaux_moeManager->findById($id);
            $contrat_bureau_etude = $this->Contrat_beManager->findById($demande_debut_travaux_moe->id_contrat_bureau_etude);
            $tranche_d_debut_travaux_moe = $this->Tranche_d_debut_travaux_moeManager->findById($demande_debut_travaux_moe->id_tranche_d_debut_travaux_moe);
            $data['id'] = $demande_debut_travaux_moe->id;
            $data['objet'] = $demande_debut_travaux_moe->objet;
            $data['description'] = $demande_debut_travaux_moe->description;
            $data['ref_facture'] = $demande_debut_travaux_moe->ref_facture;
            $data['montant'] = $demande_debut_travaux_moe->montant;
            $data['tranche_d_debut_travaux_moe'] = $tranche_d_debut_travaux_moe;
            $data['cumul'] = $demande_debut_travaux_moe->cumul;
            $data['anterieur'] = $demande_debut_travaux_moe->anterieur;
            $data['reste'] = $demande_debut_travaux_moe->reste;
            $data['date'] = $demande_debut_travaux_moe->date;
            $data['contrat_bureau_etude'] = $contrat_bureau_etude;
        } 
        else 
        {
            $menu = $this->Demande_debut_travaux_moeManager->findAll();
            if ($menu) 
            {
        ...
        if ($supprimer == 0) {
            if ($id == 0) {
                $data = array(
                    ...
                    'id_tranche_d_debut_travaux_moe' => $this->post('id_tranche_d_debut_travaux_moe'),
                ...
                $dataId = $this->Demande_debut_travaux_moeManager->add($data);
                if (!is_null($dataId)) {
                    $this->response([
                        'status' => TRUE,
                        'response' => $dataId,
                ...
                $update = $this->Demande_debut_travaux_moeManager->update($id, $data);
                if(!is_null($update)) {
                    $this->response([
                        'status' => TRUE,
                        'response' => 1,
            ...
            $delete = $this->Demande_debut_travaux_moeManager->delete($id);         
            if (!is_null($delete)) {
                $this->response([
                    'status' => TRUE,
                    'response' => 1,
