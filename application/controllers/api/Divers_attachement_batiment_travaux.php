...
class Divers_attachement_batiment_travaux extends REST_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('divers_attachement_batiment_travaux_model', 'Divers_attachement_batiment_travauxManager');
    ...
    public function index_get() 
    {
       ...
       /* if ($menu == "getmax_1attachement_travauxByattachement_prevu")
        ...
        {
            $data = array();
            $divers_attachement_batiment_travaux = $this->Divers_attachement_batiment_travauxManager->findById($id);
            $data['id'] = $divers_attachement_batiment_travaux->id;
            $data['montant_prevu'] = $divers_attachement_batiment_travaux->montant_prevu;
            $data['id_contrat_prestataire'] = $divers_attachement_batiment_travaux->id_contrat_prestataire;
            $data['id_divers_attachement_batiment'] = $divers_attachement_batiment_travaux->id_divers_attachement_batiment;
        } 
        else 
        {
            $tmp = $this->Divers_attachement_batiment_travauxManager->findAll();
            if ($tmp) 
            {
                $data=$tmp;
            } 
    ...
    public function index_post() 
    {
        ...
        if ($supprimer == 0) {
            if ($id == 0) {
                ...
                $dataId = $this->Divers_attachement_batiment_travauxManager->add($data);
                if (!is_null($dataId)) {
                    $this->response([
                        'status' => TRUE,
                        'response' => $dataId,
                ...
                $update = $this->Divers_attachement_batiment_travauxManager->update($id, $data);
                if(!is_null($update)) {
                    $this->response([
                        'status' => TRUE,
                        'response' => 1,
            ...
            $delete = $this->Divers_attachement_batiment_travauxManager->delete($id);         
            if (!is_null($delete)) {
                $this->response([
                    'status' => TRUE,
                    'response' => 1,
