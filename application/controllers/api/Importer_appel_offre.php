...
class Importer_appel_offre extends CI_Controller {
	...
	public function importerdonnee_appel_offre() {
		...
		if (isset($_FILES['file']['tmp_name']))
		{
			...
			if(!$this->upload->do_upload('file'))
			{
				...
				$rapport['appel_offre_inserer']=$retour['appel_offre_inserer'];
	...
	public function controler_donnees_importer($filename,$directory) {
		...
		{			
			$ligne = $row->getRowIndex ();
				...
				if($ligne >=3)
				{
					...
					if($erreur==false)
					{	
						$doublon = $this->Appel_offreManager->getappel_offreBycontrat($id_contrat_bureau_etude);
						if (count($doublon)>0)
						{
						...
						else
						{
		                	...
		                	array_push($appel_offre_inserer, $data);
		...
		$report['appel_offre_inserer']=$appel_offre_inserer;
