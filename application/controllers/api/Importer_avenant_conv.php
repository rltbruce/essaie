...
class Importer_avenant_conv extends CI_Controller {
	...
	public function importerdonnee_avenant_conv() {
		...
		if (isset($_FILES['file']['tmp_name']))
		{
			...
			if(!$this->upload->do_upload('file'))
			{
				...
				$rapport['avenant_conv_inserer']=$retour['avenant_conv_inserer'];
	...
	public function controler_donnees_importer($filename,$directory) {
		...
		{			
			$ligne = $row->getRowIndex ();
				...
				if($ligne >=3)
				{
					...
					if($erreur==false)
					{	
						$doublon = $this->Avenant_conventionManager->findByconventionRef_avenant($id_convention_entete,$ref_avenant);
						if (count($doublon)>0)
						{
						...
						else
						{
		                	...
		                	array_push($avenant_conv_inserer, $data);
		...
		$report['avenant_conv_inserer']=$avenant_conv_inserer;
