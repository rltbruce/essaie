...
class Importer_contrat_moe extends CI_Controller {
	...
	public function importerdonnee_contrat_moe() {
			...
			$ligne = $row->getRowIndex ();
				...
				if($ligne >=3)
				{
					...
					else
					{
						...
						$retour_bureau = $this->Bureau_etudeManager->findBynom($nom_consultant);
						if(count($retour_bureau) >0)
						{
							foreach($retour_bureau as $k=>$v)
							{
					...
					if($erreur==false)
					{	
						$doublon = $this->Contrat_beManager->findcontratByconvention($id_convention_entete);
						if (count($doublon)>0)
						{
