...
class Importer_ecoleacces extends CI_Controller {
	...
	public function testecoleacces() {
		...
		{			
			...
			$erreur = false;
				if($ligne >=10)
				{
					...
					{
						if ($acc=="HPR1" || $acc=="HPR 1")
							{
								
								$retour_subvention = $this->Zone_subventionManager->getzone_subventionbynom("HPR");
								if(count($retour_subvention) >0)
								{
									foreach($retour_subvention as $k=>$v)
									{
										$id_zone = $v->id;										
									}	
								}
								...
								$retour_acces = $this->Acces_zoneManager->getzone_accesbynom("1");
								if(count($retour_acces) >0)
								{
									foreach($retour_acces as $k=>$v)
									{
										$id_acces = $v->id;										
									}	
								}
							...
							if ($acc=="HPR2" || $acc=="HPR 2")
							{
								...
								$retour_acces = $this->Acces_zoneManager->getzone_accesbynom("2");
								if(count($retour_acces) >0)
								{
									foreach($retour_acces as $k=>$v)
									{
							...
							if ($acc=="HPR3" || $acc=="HPR 3")
							{
								...
								$retour_acces = $this->Acces_zoneManager->getzone_accesbynom("3");
								if(count($retour_acces) >0)
								{
									foreach($retour_acces as $k=>$v)
									{
							...
							{
								$retour_subvention = $this->Zone_subventionManager->getzone_subventionbynom("HPU");
								if(count($retour_subvention) >0)
								{
									foreach($retour_subvention as $k=>$v)
									{
							...
							if ($acc=="CO1" || $acc=="CO 1")
							{
								$retour_subvention = $this->Zone_subventionManager->getzone_subventionbynom("CO");
								if(count($retour_subvention) >0)
								{
									foreach($retour_subvention as $k=>$v)
									{
					...
					{							
							$data = array(
								...
								'longitude'     =>      $longitude,
								'altitude'      =>      $altitude,
								'id_zone_subvention' => $id_zone,
								'id_acces_zone' => $id_acces,
