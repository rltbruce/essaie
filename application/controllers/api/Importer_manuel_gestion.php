...
class Importer_manuel_gestion extends CI_Controller {
	...
	public function importerdonnee_manuel_gestion() {
		...
		if (isset($_FILES['file']['tmp_name']))
		{
			...
			if(!$this->upload->do_upload('file'))
			{
				...
				$rapport['manuel_gestion_inserer']=$retour['manuel_gestion_inserer'];
	...
	public function controler_donnees_importer($filename,$directory) {
		...
		{			
			$ligne = $row->getRowIndex ();
				...
				if($ligne >=3)
				{
					...
					if($erreur==false)
					{	
						$doublon = $this->Manuel_gestionManager->getmanuel_gestionBycontrat($id_contrat_bureau_etude);
						if (count($doublon)>0)
						{
						...
						else
						{
		                	...
		                	array_push($manuel_gestion_inserer, $data);
		...
		$report['manuel_gestion_inserer']=$manuel_gestion_inserer;
