...
class Importer_module_dpp extends CI_Controller {
	...
	public function importerdonnee_module_dpp() {
		...
		if (isset($_FILES['file']['tmp_name']))
		{
			...
			if(!$this->upload->do_upload('file'))
			{
				...
				$rapport['module_dpp_inserer']=$retour['module_dpp_inserer'];
	...
	public function controler_donnees_importer($filename,$directory) {
		...
		{			
			$ligne = $row->getRowIndex ();
				...
				if($ligne >=3)
				{
					...
					if($erreur==false)
					{	
						$doublon = $this->Module_dppManager->getmoduleBycontrat($id_contrat_partenaire_relai);
						if (count($doublon)>0)
						{
						...
						else
						{
		                	...
		                	array_push($module_dpp_inserer, $data);
		...
		$report['module_dpp_inserer']=$module_dpp_inserer;
