...
class Importer_module_gfpc extends CI_Controller {
	...
	public function importerdonnee_module_gfpc() {
		...
		if (isset($_FILES['file']['tmp_name']))
		{
			...
			if(!$this->upload->do_upload('file'))
			{
				...
				$rapport['module_gfpc_inserer']=$retour['module_gfpc_inserer'];
	...
	public function controler_donnees_importer($filename,$directory) {
		...
		{			
			$ligne = $row->getRowIndex ();
				...
				if($ligne >=3)
				{
					...
					if($erreur==false)
					{	
						$doublon = $this->Module_gfpcManager->getmoduleBycontrat($id_contrat_partenaire_relai);
						if (count($doublon)>0)
						{
						...
						else
						{
		                	...
		                	array_push($module_gfpc_inserer, $data);
		...
		$report['module_gfpc_inserer']=$module_gfpc_inserer;
