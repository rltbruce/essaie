...
class Importer_module_odc extends CI_Controller {
	...
	public function importerdonnee_module_odc() {
		...
		if (isset($_FILES['file']['tmp_name']))
		{
			...
			if(!$this->upload->do_upload('file'))
			{
				...
				$rapport['module_odc_inserer']=$retour['module_odc_inserer'];
	...
	public function controler_donnees_importer($filename,$directory) {
		...
		{			
			$ligne = $row->getRowIndex ();
				...
				if($ligne >=3)
				{
					...
					if($erreur==false)
					{	
						$doublon = $this->Module_odcManager->getmoduleBycontrat($id_contrat_partenaire_relai);
						if (count($doublon)>0)
						{
						...
						else
						{
		                    ...
		                    'id_classification_site'   => null, 
		                    'validation' => 0
		                	);
	                		$dataId = $this->Module_odcManager->add($data);                		

		                	array_push($module_odc_inserer, $data);
		...
		$report['module_odc_inserer']=$module_odc_inserer;
