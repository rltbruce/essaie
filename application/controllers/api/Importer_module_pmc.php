...
class Importer_module_pmc extends CI_Controller {
	...
	public function importerdonnee_module_pmc() {
		...
		if (isset($_FILES['file']['tmp_name']))
		{
			...
			if(!$this->upload->do_upload('file'))
			{
				...
				$rapport['module_pmc_inserer']=$retour['module_pmc_inserer'];
	...
	public function controler_donnees_importer($filename,$directory) {
		...
		{			
			$ligne = $row->getRowIndex ();
				...
				if($ligne >=3)
				{
					...
					if($erreur==false)
					{	
						$doublon = $this->Module_pmcManager->getmoduleBycontrat($id_contrat_partenaire_relai);
						if (count($doublon)>0)
						{
						...
						else
						{
		                	...
		                	array_push($module_pmc_inserer, $data);
		...
		$report['module_pmc_inserer']=$module_pmc_inserer;
