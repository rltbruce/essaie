...
class Importer_module_sep extends CI_Controller {
	...
	public function importerdonnee_module_sep() {
		...
		if (isset($_FILES['file']['tmp_name']))
		{
			...
			if(!$this->upload->do_upload('file'))
			{
				...
				$rapport['module_sep_inserer']=$retour['module_sep_inserer'];
	...
	public function controler_donnees_importer($filename,$directory) {
		...
		{			
			$ligne = $row->getRowIndex ();
				...
				if($ligne >=3)
				{
					...
					if($erreur==false)
					{	
						$doublon = $this->Module_sepManager->getmoduleBycontrat($id_contrat_partenaire_relai);
						if (count($doublon)>0)
						{
						...
						else
						{
		                	...
		                	array_push($module_sep_inserer, $data);
		...
		$report['module_sep_inserer']=$module_sep_inserer;
