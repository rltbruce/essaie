...
class Importer_participant_dpp extends CI_Controller {
	...
	public function importerdonnee_participant_dpp() {
		...
		if (isset($_FILES['file']['tmp_name']))
		{
			...
			if(!$this->upload->do_upload('file'))
			{
				...
				$rapport['participant_dpp_inserer']=$retour['participant_dpp_inserer'];
	...
	public function controler_donnees_importer($filename,$directory) {
			...
			$ligne = $row->getRowIndex ();
				...
				{	
					...
					else
					{
						...
						$retour_module = $this->Module_dppManager->findByRef_convention($ref_convention);
				...
				if($ligne >=4)
				{
					...
					{	
						...
						$retour_fonction = $this->Situation_participant_dppManager->findByFonction($fonction);
					...
					if($erreur==false)
					{	
						$doublon = $this->Participant_dppManager->getparticipantBymodulefonction($id_module_dpp,$id_fonction);
						if (count($doublon)>0)
						{
						...
						else
						{
		        			...
		        			$data = array(
		                    'id_module_dpp' => $id_module_dpp,
		                    'nom' => $nom,
		                    'prenom'   => $prenom,
		                    'sexe'    => $statu_sexe,
		                    'id_situation_participant_dpp' => $id_fonction
		                	);
	                		$dataId = $this->Participant_dppManager->add($data);                		

		                	array_push($participant_dpp_inserer, $data);
		...
		$report['participant_dpp_inserer']=$participant_dpp_inserer;
