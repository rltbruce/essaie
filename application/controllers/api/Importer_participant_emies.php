...
class Importer_participant_emies extends CI_Controller {
	...
	public function importerdonnee_participant_emies() {
		...
		if (isset($_FILES['file']['tmp_name']))
		{
			...
			if(!$this->upload->do_upload('file'))
			{
				...
				$rapport['participant_emies_inserer']=$retour['participant_emies_inserer'];
	...
	public function controler_donnees_importer($filename,$directory) {
		...
		$nbr_erreur=0;
		$nbr_inserer=0;
		$participant_emies_inserer = array();
		//The name of the directory that we need to create.
		$directoryName = dirname(__FILE__) ."/../../../../../../assets/excel/".$repertoire;
		//Check if the directory already exists.
		if(!is_dir($directoryName)){
			...
			$ligne = $row->getRowIndex ();
				...
				{	
					...
					else
					{
						...
						$retour_module = $this->Module_emiesManager->findByRef_convention($ref_convention);
						if(count($retour_module) >0)
						{
							foreach($retour_module as $k=>$v)
							{
								$id_module_emies = $v->id;
				...
				if($ligne >=4)
				{
					...
					{	
						...
						$retour_fonction = $this->Situation_participant_emiesManager->findByFonction($fonction);
					...
					if($erreur==false)
					{	
						$doublon = $this->Participant_emiesManager->getparticipantBymodulefonction($id_module_emies,$id_fonction);
						if (count($doublon)>0)
						{
						...
						else
						{
		        			...
		        			$data = array(
		                    'id_module_emies' => $id_module_emies,
		                    'nom' => $nom,
		                    'prenom'   => $prenom,
		                    'sexe'    => $statu_sexe,
		                    'id_situation_participant_emies' => $id_fonction
		                	);
	                		$dataId = $this->Participant_emiesManager->add($data);                		

		                	array_push($participant_emies_inserer, $data);
		...
		$report['participant_emies_inserer']=$participant_emies_inserer;
