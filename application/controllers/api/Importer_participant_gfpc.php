...
class Importer_participant_gfpc extends CI_Controller {
	...
	public function importerdonnee_participant_gfpc() {
		...
		if (isset($_FILES['file']['tmp_name']))
		{
			...
			if(!$this->upload->do_upload('file'))
			{
				...
				$rapport['participant_gfpc_inserer']=$retour['participant_gfpc_inserer'];
	...
	public function controler_donnees_importer($filename,$directory) {
		...
		$nbr_erreur=0;
		$nbr_inserer=0;
		$participant_gfpc_inserer = array();
		//The name of the directory that we need to create.
		$directoryName = dirname(__FILE__) ."/../../../../../../assets/excel/".$repertoire;
		//Check if the directory already exists.
		if(!is_dir($directoryName)){
			...
			$ligne = $row->getRowIndex ();
				...
				{	
					...
					else
					{
						...
						$retour_module = $this->Module_gfpcManager->findByRef_convention($ref_convention);
				...
				if($ligne >=4)
				{
					...
					{	
						...
						$retour_fonction = $this->Situation_participant_gfpcManager->findByFonction($fonction);
					...
					if($erreur==false)
					{	
						$doublon = $this->Participant_gfpcManager->getparticipantBymodulefonction($id_module_gfpc,$id_fonction);
						if (count($doublon)>0)
						{
						...
						else
						{
		        			...
		        			$data = array(
		                    'id_module_gfpc' => $id_module_gfpc,
		                    'nom' => $nom,
		                    'prenom'   => $prenom,
		                    'sexe'    => $statu_sexe,
		                    'id_situation_participant_gfpc' => $id_fonction
		                	);
	                		$dataId = $this->Participant_gfpcManager->add($data);                		

		                	array_push($participant_gfpc_inserer, $data);
		...
		$report['participant_gfpc_inserer']=$participant_gfpc_inserer;
