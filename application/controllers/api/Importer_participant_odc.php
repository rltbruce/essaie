...
class Importer_participant_odc extends CI_Controller {
	...
	public function importerdonnee_participant_odc() {
		...
		if (isset($_FILES['file']['tmp_name']))
		{
			...
			if(!$this->upload->do_upload('file'))
			{
				...
				$rapport['participant_odc_inserer']=$retour['participant_odc_inserer'];
	...
	public function controler_donnees_importer($filename,$directory) {
			...
			$ligne = $row->getRowIndex ();
				...
				{	
					...
					else
					{
						...
						$retour_module = $this->Module_odcManager->findByRef_convention($ref_convention);
				...
				if($ligne >=4)
				{
					...
					{	
						...
						$retour_fonction = $this->Situation_participant_odcManager->findByFonction($fonction);
					...
					if($erreur==false)
					{	
						$doublon = $this->Participant_odcManager->getparticipantBymodulefonction($id_module_odc,$id_fonction);
						if (count($doublon)>0)
						{
						...
						else
						{
		        			...
		        			$data = array(
		                    'id_module_odc' => $id_module_odc,
		                    'nom' => $nom,
		                    'prenom'   => $prenom,
		                    'sexe'    => $statu_sexe,
		                    'id_situation_participant_odc' => $id_fonction
		                	);
	                		$dataId = $this->Participant_odcManager->add($data);                		

		                	array_push($participant_odc_inserer, $data);
		...
		$report['participant_odc_inserer']=$participant_odc_inserer;
