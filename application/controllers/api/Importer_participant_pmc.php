...
class Importer_participant_pmc extends CI_Controller {
	...
	public function importerdonnee_participant_pmc() {
		...
		if (isset($_FILES['file']['tmp_name']))
		{
			...
			if(!$this->upload->do_upload('file'))
			{
				...
				$rapport['participant_pmc_inserer']=$retour['participant_pmc_inserer'];
	...
	public function controler_donnees_importer($filename,$directory) {
			...
			$ligne = $row->getRowIndex ();
				...
				{	
					...
					else
					{
						...
						$retour_module = $this->Module_pmcManager->findByRef_convention($ref_convention);
				...
				if($ligne >=4)
				{
					...
					{	
						...
						$retour_fonction = $this->Situation_participant_pmcManager->findByFonction($fonction);
					...
					if($erreur==false)
					{	
						$doublon = $this->Participant_pmcManager->getparticipantBymodulefonction($id_module_pmc,$id_fonction);
						if (count($doublon)>0)
						{
						...
						else
						{
		        			...
		        			$data = array(
		                    'id_module_pmc' => $id_module_pmc,
		                    'nom' => $nom,
		                    'prenom'   => $prenom,
		                    'sexe'    => $statu_sexe,
		                    'id_situation_participant_pmc' => $id_fonction
		                	);
	                		$dataId = $this->Participant_pmcManager->add($data);                		

		                	array_push($participant_pmc_inserer, $data);
		...
		$report['participant_pmc_inserer']=$participant_pmc_inserer;
