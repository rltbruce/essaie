...
class Importer_passation_moe extends CI_Controller {
	...
	public function importerdonnee_passation_moe() {
			...
			$ligne = $row->getRowIndex ();
				...
				if($ligne >=3)
				{
					...
					foreach ($cellIterator as $cell)
					{
						...
						{
							$date_shortlist =$cell->getValue();
							if(isset($date_shortlist) && $date_shortlist>"")
							{
								if(PHPExcel_Shared_Date::isDateTime($cell))
								{
									$date_shortlist = date($format='Y-m-d', PHPExcel_Shared_Date::ExcelToPHP($date_shortlist)); 
								}
							} 
							else 
							{
						...
						{
							$date_signature_contrat = $cell->getValue();
							if(isset($date_signature_contrat) && $date_signature_contrat>"")
							{
								if(PHPExcel_Shared_Date::isDateTime($cell))
								{
									$date_signature_contrat = date($format='Y-m-d', PHPExcel_Shared_Date::ExcelToPHP($date_signature_contrat)); 
								}
							} 
							else 
							{
					...
					if($erreur==false)
					{	
						$doublon = $this->Passation_marches_beManager->findpassationByconvention($id_convention_entete);
						if (count($doublon)>0)
						{
							$sheet->setCellValue('P'.$ligne, "Doublon");
							$nbr_erreur = $nbr_erreur + 1;							
						}
						else
						{
		        			$sheet->setCellValue('p'.$ligne, "Ok");
		        			$data = array(
					...
					{
						$sheet->setCellValue('P'.$ligne, "Erreur");
						$nbr_erreur = $nbr_erreur + 1;
					}						
