...
class Importer_police_assurance extends CI_Controller {
	...
	public function importerdonnee_police_assurance() {
		...
		if (isset($_FILES['file']['tmp_name']))
		{
			...
			if(!$this->upload->do_upload('file'))
			{
				...
				$rapport['police_assurance_inserer']=$retour['police_assurance_inserer'];
	...
	public function controler_donnees_importer($filename,$directory) {
		...
		$nbr_erreur=0;
		$nbr_inserer=0;
		$police_assurance_inserer = array();
		//The name of the directory that we need to create.
		$directoryName = dirname(__FILE__) ."/../../../../../../assets/excel/".$repertoire;
		//Check if the directory already exists.
		if(!is_dir($directoryName)){
			...
			$ligne = $row->getRowIndex ();
				...
				if($ligne >=3)
				{
					...
					foreach ($cellIterator as $cell)
					{
						...
						{
							$date_expiration =$cell->getValue();
							if(isset($date_expiration) && $date_expiration>"")
							{
								if(PHPExcel_Shared_Date::isDateTime($cell))
								{
									$date_expiration = date($format='Y-m-d', PHPExcel_Shared_Date::ExcelToPHP($date_expiration)); 
								}
							} 
							else 
							{
					...
					if($erreur==false)
					{	
						$doublon = $this->Police_assuranceManager->getpolice_assuranceBycontrat($id_contrat_bureau_etude);
						if (count($doublon)>0)
						{
						...
						else
						{
		                	...
		                	array_push($police_assurance_inserer, $data);
		...
		$report['police_assurance_inserer']=$police_assurance_inserer;
