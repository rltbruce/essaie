...
class Module_gfpc extends REST_Controller {
    ...
    public function index_get() 
    {
        ...
        if ($menu=='getmoduleBycontrat')
         {
            $tmp = $this->Module_gfpcManager->findmoduleBycontrat($id_contrat_partenaire_relai);
            if ($tmp) 
            {
                foreach ($tmp as $key => $value) 
                {
                    ...
                    $nbr_parti= $this->Participant_gfpcManager->count_participantbyId($value->id);
                    $nbr_feminin= $this->Participant_gfpcManager->count_femininbyId($value->id);
        ...
        elseif ($menu=='getmodulevalideBycontrat')
         {
            $tmp = $this->Module_gfpcManager->findvalideBycontrat($id_contrat_partenaire_relai);
            if ($tmp) 
            {
                foreach ($tmp as $key => $value) 
                {
        ...
        elseif ($menu=='getmodulevalideById')
         {
            $tmp = $this->Module_gfpcManager->findvalideById($id_module);
            if ($tmp) 
            {
                foreach ($tmp as $key => $value) 
                {
        ...
        elseif ($menu=='getmoduleinvalideBycontrat')
         {
            $tmp = $this->Module_gfpcManager->findinvalideBycontrat($id_contrat_partenaire_relai);
            if ($tmp) 
            {
                foreach ($tmp as $key => $value) 
                {
        ...
        /* if ($menu=='getmodule_gfpcByinvalide')
         {
            $menu = $this->Module_gfpcManager->findAllByinvalide();
            if ($menu) 
            {
        ...
        elseif ($menu=='getmodule_gfpcByDate')
         {
            $menu = $this->Module_gfpcManager->findAllBydate();
            if ($menu) 
            {
                foreach ($menu as $key => $value) 
                {
        ...
        elseif ($menu=='getmodule_gfpcByinvalide')
         {
            $menu = $this->Module_gfpcManager->findAllByinvalide();
            if ($menu) 
            {
        ...
        elseif ($menu=='getmodule_gfpcBycontrat')
         {
            $menu = $this->Module_gfpcManager->findAllBycontrat($id_contrat_partenaire_relai);
            if ($menu) 
            {
                foreach ($menu as $key => $value) 
                {
        ...
        {
            $data = array();
            $module_gfpc = $this->Module_gfpcManager->findById($id);

            //$contrat_partenaire_relai = $this->Contrat_partenaire_relaiManager->findById($module_gfpc->id_contrat_partenaire_relai);
            $nbr_parti= $this->Participant_gfpcManager->count_participantbyId($module_gfpc->id);
            $nbr_feminin= $this->Participant_gfpcManager->count_femininbyId($module_gfpc->id);

            $data['id'] = $module_gfpc->id;
            $data['date_debut_previ_form'] = $module_gfpc->date_debut_previ_form;
            $data['date_fin_previ_form']   = $module_gfpc->date_fin_previ_form;
            $data['date_previ_resti']    = $module_gfpc->date_previ_resti;
            $data['date_debut_reel_form'] = $module_gfpc->date_debut_reel_form;
            $data['date_fin_reel_form'] = $module_gfpc->date_fin_reel_form;
            $data['date_reel_resti'] = $module_gfpc->date_reel_resti;
            $data['nbr_previ_parti']   = $module_gfpc->nbr_previ_parti;
            $data['nbr_parti']    = $nbr_parti->nbr_participant;
            $data['nbr_previ_fem_parti']   = $module_gfpc->nbr_previ_fem_parti;
            $data['nbr_reel_fem_parti'] = $nbr_feminin->nbr_feminin;
            $data['lieu_formation'] = $module_gfpc->lieu_formation;
            $data['observation']   = $module_gfpc->observation;
            //$data['contrat_partenaire_relai'] = $contrat_partenaire_relai;
        } 
        else 
        {
            $menu = $this->Module_gfpcManager->findAll();
            if ($menu) 
            {
                foreach ($menu as $key => $value) 
                {
    ...
    public function index_post() 
    {
        ...
        if ($supprimer == 0) {
            if ($id == 0) {
                ...
                $dataId = $this->Module_gfpcManager->add($data);
                if (!is_null($dataId)) {
                    $this->response([
                        'status' => TRUE,
                        'response' => $dataId,
                ...
                $update = $this->Module_gfpcManager->update($id, $data);
                if(!is_null($update)) {
                    $this->response([
                        'status' => TRUE,
                        'response' => 1,
            ...
            $delete = $this->Module_gfpcManager->delete($id);         
            if (!is_null($delete)) {
                $this->response([
                    'status' => TRUE,
                    'response' => 1,
