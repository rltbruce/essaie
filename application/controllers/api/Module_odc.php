...
class Module_odc extends REST_Controller {
    ...
    public function index_get() 
    {
        ...
        elseif ($menu=='getmodule_odcByDate')
         {
            $menu = $this->Module_odcManager->findAllBydate();
            if ($menu) 
            {
                foreach ($menu as $key => $value) 
                {
        ...
        elseif ($menu=='getmodule_odcBycontrat')
         {
            $menu = $this->Module_odcManager->findAllBycontrat($id_contrat_partenaire_relai);
            if ($menu) 
            {
                foreach ($menu as $key => $value) 
                {
        ...
        elseif ($id)
        {
            ...
            $data['id'] = $module_odc->id;
            $data['date_debut_previ_form'] = $module_odc->date_debut_previ_form;
            $data['date_fin_previ_form']   = $module_odc->date_fin_previ_form;
            $data['date_previ_resti']    = $module_odc->date_previ_resti;
            $data['date_debut_reel_form'] = $module_odc->date_debut_reel_form;
            $data['date_fin_reel_form'] = $module_odc->date_fin_reel_form;
            $data['date_reel_resti'] = $module_odc->date_reel_resti;
            $data['nbr_previ_parti']   = $module_odc->nbr_previ_parti;
            $data['nbr_parti']    = $nbr_parti->nbr_participant;
            $data['nbr_previ_fem_parti']   = $module_odc->nbr_previ_fem_parti;
        ...
        {
            $menu = $this->Module_odcManager->findAll();
            if ($menu) 
            {
                foreach ($menu as $key => $value) 
                {
    ...
    public function index_post() 
    {
        ...
        if ($supprimer == 0) {
            if ($id == 0) {
                ...
                $dataId = $this->Module_odcManager->add($data);
                if (!is_null($dataId)) {
                    $this->response([
                        'status' => TRUE,
                        'response' => $dataId,
                ...
                $update = $this->Module_odcManager->update($id, $data);
                if(!is_null($update)) {
                    $this->response([
                        'status' => TRUE,
                        'response' => 1,
            ...
            $delete = $this->Module_odcManager->delete($id);         
            if (!is_null($delete)) {
                $this->response([
                    'status' => TRUE,
                    'response' => 1,
