...
class Document_feffi_model extends CI_Model {
    protected $table = 'document_feffi';

    public function add($document_feffi) {
        $this->db->set($this->_set($document_feffi))
                            ->insert($this->table);
    ...
    public function update($id, $document_feffi) {
        $this->db->set($this->_set($document_feffi))
                            ->where('id', (int) $id)
                            ->update($this->table);
        if($this->db->affected_rows() === 1)
    ...
    public function _set($document_feffi) {
        return array(
