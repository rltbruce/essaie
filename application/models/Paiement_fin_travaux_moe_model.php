...
class Paiement_fin_travaux_moe_model extends CI_Model {
    protected $table = 'paiement_fin_travaux_moe';

    public function add($paiement_fin_travaux_moe) {
        $this->db->set($this->_set($paiement_fin_travaux_moe))
                            ->insert($this->table);
    ...
    public function update($id, $paiement_fin_travaux_moe) {
        $this->db->set($this->_set($paiement_fin_travaux_moe))
                            ->where('id', (int) $id)
                            ->update($this->table);
        if($this->db->affected_rows() === 1)
    ...
    public function _set($paiement_fin_travaux_moe) {
        return array(
